...
class TestCopilotSdkWrapper(unittest.TestCase):
    ...
    def test_create_copilot_session_falls_back_to_latest_supported_family_model(
        self,
    ) -> None:
        client = SimpleNamespace(
            list_models=AsyncMock(
                return_value=[
                    SimpleNamespace(id="gpt-5.2", name="GPT-5.2"),
                    SimpleNamespace(id="gpt-5.4", name="GPT-5.4"),
                    SimpleNamespace(id="gpt-5-mini", name="GPT-5 mini"),
                ]
            ),
