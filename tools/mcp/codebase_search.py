"""Codebase-search MCP server for EventTracker — AST-based structured search.

Parses every Python module into an AST and answers questions about code
**structure** — definitions, references, and module shape. Never
substring-matches: grep handles text; this handles structure.
...
Run directly:  uv run --dev python tools/mcp/codebase_search.py
"""
...
# Resolve against the project root. Works both when run directly (cwd) and
# when a host sets COPILOT_PROJECT_DIR or CLAUDE_PROJECT_DIR.
ROOT = Path(
...
# Dependencies, caches, build output, and tooling dirs are never "the
# codebase" — skip them so the index covers repo source only.
EXCLUDE_DIRS = frozenset({
    ".git", ".venv", "venv", "env", "node_modules", "__pycache__",
    ".pytest_cache", ".mypy_cache", ".ruff_cache", "build", "dist",
    ".claude", ".agents", ".github", ".tox", "site-packages",
})

mcp = FastMCP("eventtracker-codebase-search")
...
def _python_files() -> list[Path]:
    ...
    directories. Uses a pruning walk so it never descends into excluded dirs."""
    files: list[Path] = []
...
class _DefCollector(ast.NodeVisitor):
    """Collects every definition in one module, tracking the enclosing
    class/function stack so qualified names are accurate."""
...
def find_references(name: str) -> str:
    ...
    Returns only genuine references, not every textual mention.
    """
...
def outline(module: str) -> str:
    ...
    `module` accepts a path (`app/services/entries.py`) or a dotted name
    (`app.services.entries`, or just `entries`).
    """
