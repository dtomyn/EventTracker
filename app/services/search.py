...
def _entry_sort_key(rows_by_id: dict[int, sqlite3.Row], entry_id: int) -> tuple[int, str]:
    row = rows_by_id[entry_id]
    return row["sort_key"], row["updated_utc"]
...
def filter_timeline_entries(
    ...
    if tag_matched_ids:
        ...
        sorted_ids = sorted(
            tag_matched_ids,
            key=lambda entry_id: _entry_sort_key(rows_by_id, entry_id),
            reverse=True,
        )
