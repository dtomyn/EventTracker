...
- FR-008-02 JSON export shall include a top-level `count` and `entries` array.
- FR-008-03 Each exported entry shall include saved tags, additional links, and display-oriented metadata needed to reconstruct the saved record.
- FR-008-04 Exported files shall use a timestamped filename prefixed with `EventTracker-export-`.
...
- FR-008-06 Import shall skip exact duplicate entries by default and shall support an `--allow-duplicates` override.
- FR-008-07 JSON import shall validate exported tag and link structure before writing any imported row.
- FR-008-08 The repository shall provide `scripts/init_db.py` to initialize required database structures.
- FR-008-09 `scripts/init_db.py` shall support `--reindex-embeddings` to rebuild sqlite-vec embeddings when embedding configuration is valid.
